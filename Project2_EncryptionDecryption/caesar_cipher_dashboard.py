...
class CipherDashboard(tk.Tk):
    ...
    def _build_vigenere_frame(self):
        ...
        tk.Label(inner, text="📝  TEXT", font=("Times New Roman", 11, "bold"),
                 bg=BG_CARD, fg=ACCENT).pack(anchor="w")
        self.v_input = tk.Text(inner, height=4, font=FONT_MONO, bg="#f2f6fb",
                                fg=TEXT_MAIN, insertbackground=TEXT_MAIN,
                                relief="flat", wrap="word")
        ...
        enc_btn.bind("<Button-1>", lambda e: self._do_vigenere(True))

        dec_btn = tk.Label(btn_row, text="  Decrypt", image=self.decrypt_btn_icon,
                            compound="left", bg="#f2f6fb", fg=TEXT_MAIN,
                            font=("Segoe UI", 11, "bold"), padx=18, pady=10, cursor="hand2")
        dec_btn.pack(side="left", padx=(10, 0))
        dec_btn.bind("<Button-1>", lambda e: self._do_vigenere(False))

        tk.Label(inner, text="📤  OUTPUT", font=("Times New Roman", 11, "bold"),
                 bg=BG_CARD, fg=ACCENT).pack(anchor="w")
        self.v_output = tk.Text(inner, height=4, font=FONT_MONO, bg="#f2f6fb",
                                 fg=GOOD_COLOR, insertbackground=TEXT_MAIN,
                                 relief="flat", wrap="word")
        self.v_output.pack(fill="x", pady=(6, 0))

    def _do_vigenere(self, encrypt):
        text = self.v_input.get("1.0", "end-1c")
        key = self.v_key_var.get()
        result = vigenere_process(text, key, encrypt=encrypt)
